...
const QuizLeaderboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] pt-32 md:pt-48 pb-40 overflow-hidden relative text-slate-800">
      <div className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-[#14B8A6]/10 rounded-full blur-[150px] pointer-events-none -mt-80 animate-pulse-soft" />
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        <div className="text-center mb-24 max-w-3xl mx-auto">
           ...
           <h1 className="text-5xl md:text-[6rem] font-bn font-black text-[#0F172A] italic leading-none mb-6 tracking-tighter">
             সেরাদের <span className="text-yellow-500 italic">তালিকা</span>
           </h1>
           <p className="text-2xl text-slate-500 font-bn italic leading-relaxed mb-12">
           ...
           <div className="flex overflow-x-auto no-scrollbar gap-3 pb-4 mb-4 justify-start md:justify-center">
              {quizSubjects.map((s) => (
                <button
                  ...
                  className={`flex-shrink-0 px-6 py-3 rounded-2xl font-bn font-black italic text-[15px] transition-all border ${activeSubject === s.id ? 'bg-[#14B8A6] text-white border-[#14B8A6] shadow-[0_10px_30px_rgba(20,184,166,0.2)]' : 'bg-[var(--bg-elevated)] text-slate-500 border-[var(--bg-border)] hover:bg-slate-100'}`}
                >
                  {s.titleBn}
                </button>
              ))}
        ...
        <div className="flex justify-center mb-16">
           <button 
             ...
             className="bg-[var(--bg-elevated)] text-[#0F172A] border border-[var(--bg-border)] hover:bg-slate-100 h-16 px-10 rounded-[2rem] font-bn font-black italic text-xl shadow-lg transition-all flex items-center gap-3 backdrop-blur-md"
           >
        ...
        <div className="bg-[var(--bg-surface)] rounded-[3rem] md:rounded-[4rem] border border-[var(--bg-border)] shadow-2xl overflow-hidden p-6 md:p-12 min-h-[400px]">
           {loading ? (
             <div className="flex flex-col items-center justify-center h-full py-32 opacity-70">
               ...
               <p className="text-xl font-bn italic text-slate-500">Loading Leaderboard...</p>
             </div>
           ) : scores.length === 0 ? (
             <div className="py-32 text-center text-slate-400 font-bn italic text-3xl font-black">
                এখনো কেউ কুইজ শেষ করেনি। আপনিই প্রথম হন!
             </div>
           ) : (
             <div className="flex flex-col gap-6">
                 {/* Header Row */}
                 <div className="hidden md:grid grid-cols-12 gap-8 px-10 py-4 text-[10px] font-black en-font tracking-[0.2em] uppercase text-slate-500 border-b border-[var(--bg-border)]">
                 ...
                 {scores.map((score, idx) => {
                   ...
                   return (
                     <motion.div 
                       ...
                       className={`grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-center px-8 md:px-10 py-6 md:py-8 rounded-[2rem] md:rounded-[2.5rem] border hover:bg-slate-50/50 transition-all duration-300 group relative overflow-hidden ${isMe ? 'bg-[#14B8A6]/10 border-[#14B8A6] shadow-[0_0_30px_rgba(20,184,166,0.15)] scale-[1.02] z-10' : 'bg-white border-[var(--bg-border)]'}`}
                     >
                       {isMe && <div className="absolute top-0 right-10 bg-[#14B8A6] text-white text-[10px] font-black uppercase px-4 py-1 rounded-b-xl shadow-lg en-font tracking-[0.2em]">You</div>}
                       ...
                       <div className="col-span-1 md:col-span-5 text-center md:text-left">
                          <h3 className="text-3xl font-bn font-black text-[#0F172A] italic mb-2 tracking-tight">{score.name}</h3>
                          <p className="flex items-center justify-center md:justify-start gap-2 text-slate-500 font-bn italic text-lg opacity-80">
                       ...
                       <div className="col-span-1 md:col-span-3 flex justify-center md:justify-start">
                          <span className={`px-4 py-1 rounded-full text-[10px] font-black en-font tracking-[0.2em] uppercase border flex items-center gap-2 shadow-sm ${score.mode === 'exam' ? 'bg-[#14B8A6]/10 text-[#14B8A6] border-[#14B8A6]/20' : 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20'}`}>
                       ...
                       <div className="col-span-1 md:col-span-2 flex justify-center flex-col items-center">
                          <div className="text-4xl md:text-5xl font-bn font-black text-[#0F172A] italic tracking-tighter">
                            {score.score}<span className="text-xl md:text-3xl text-slate-400">/{score.total}</span>
                          </div>
                          <div className="text-[#14B8A6] font-black en-font text-[9px] uppercase tracking-widest mt-1">Accuracy {Math.round((score.score/score.total)*100)}%</div>
                       </div>
                     </motion.div>
                   )
                 })}
